...
export function applyPatch(repoDir, patchPath) {
  const result = spawnSync('git', ['apply', patchPath], { cwd: repoDir, stdio: 'inherit' });
  if (result.status !== 0) {
    throw new Error(`git apply failed for ${patchPath} in ${repoDir}`);
  }
}
