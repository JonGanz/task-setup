...
import { ensureWinCache, linkNodeModulesWindows } from '../win-cache.js';
import { listPatches } from '../patches.js';
import { buildWindowName } from '../window-name.js';
...
export async function runNew(argv) {
  ...
  for (const repo of selectedRepos) {
    ...
    if (existsSync(lockFile)) {
      ...
      if (repo.npmInstallOn === 'windows') {
        const cacheDir = await ensureWinCache(hash, repoDir, config.winWorkDir);
        await linkNodeModulesWindows(repoDir, cacheDir);
      } else {
  ...
  if (mux.isInside()) {
    ...
    mux.renameAndRun(windowName, commandLine, pane);
  } else {
    console.log(`(not running inside ${config.multiplexer} — skipping window rename/cd)`);
