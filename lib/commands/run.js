...
export async function runRun(argv) {
  ...
  for (const c of chosen) {
    ...
    mux.createWindow(windowName, commandStr, cwd, logFile);

    const pid = mux.getWindowPid(windowName);
    addWindow(state, {
      repo: c.repoName,
      profile: c.profile,
      windowName,
  ...
  console.log(`\nDone. Session "${mux.SESSION_NAME}" — "task-setup status" to check, "task-setup attach <repo>" to attach.`);
}
